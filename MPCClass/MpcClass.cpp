...
umin(-1.774), umax(1.774), delta_umin(-1.48), delta_umax(1.48),
...
{}
void MyController::SendValues(double time, double Previous, double u0, double u1, double u2, double u3, double u4, double u5)
{
	U[0] = Previous;//上一次输出
	t = time;
...
/****************************************************************************************************/
MatrixXd MyController::GetDesignPath(double x, double y,double PointX[],double PointY[])
{
...
double MyController::Calculate()
{
	//计算Kesi,将状态量与控制量结合在一起
	MatrixXd Kesi(Nx + Nu, 1);
	Kesi(0, 0) = -x_dot*(tan(U[0] * 3.14*3.14 / 180));//Original value y_dot
	Kesi(1, 0) = x_dot;
	Kesi(2, 0) = phi;
	Kesi(3, 0) = phi_dot;
	Kesi(4, 0) = Y;
	...
	QuadProgPP::Matrix<double> NoA_cons;
	QuadProgPP::Vector<double> NoB_cons;
...
///计算克罗内克积
///
MatrixXd MyController::Kron(MatrixXd A, MatrixXd B)
{
...
///对角元设置
///
void MyController::Diag_Mat(MatrixXd &rst, MatrixXd innerMat, int num)
{
	for (int i = 0; i < num; i++)
	{
...
///矩阵的幂次方
///
MatrixXd MyController::Pow_Mat(MatrixXd x, int y)
{
...
///实现四个矩阵合并
///
void MyController::Mer_Mat(MatrixXd &dsttmp, MatrixXd &src11, MatrixXd &src12, MatrixXd &src21, MatrixXd &src22)
{
...
///纵向四个矩阵合并
///
void MyController::Mer_Vertical(MatrixXd &dsttmp, MatrixXd &src1, MatrixXd &src2, MatrixXd &src3, MatrixXd &src4)
{
...
void MyController::Mer_Demo(void)
{
	//合成 5 X 4 矩阵
	MatrixXd X11(3, 3);
	...
	cout << "Here is the Matrix X11:\n" << X11 << "\n"
		<< "Here is the Matrix X12:\n" << X12 << "\n"
		<< "Here is the Matrix X21:\n" << X21 << "\n"
		<< "Here is the Matrix X22:\n" << X22 << endl;

	cout << "Here is the Mer_Mat_tmp:\n" << Mer_Mat_tmp << endl;
}
...
///两个库之间模板的转换
///
void MyController::EigenToMatrix(Eigen::MatrixXd &src, QuadProgPP::Matrix<double> &dst)
{
...
void MyController::EigenToVector(Eigen::MatrixXd &src, QuadProgPP::Vector<double> &dst)
{
